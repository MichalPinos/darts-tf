...
from config import FLAGS as FLAGS
import math
import builtins
import json
...
def main(argv):
    ...
    with open("/home/michalpinos/darts_tf2/darts-tf2/genotypes/og", "r") as f:
        genotype_str = f.read()
    print(genotype_str)
    genotype = builtins.eval(genotype_str)
    
    config["genotype"] = genotype_str
    config["logdir"] = FLAGS.logdir
    config["gpus"] = FLAGS.gpus
    config["load_weights"] = FLAGS.load_weights    
    ## dump used config, no modifications to config after this!
    with open(FLAGS.logdir + "/config.json", "w") as f:
        json.dump(config, f)
    
    # Set strategy
    devices = [f"/device:gpu:{i}" for i in range(FLAGS.gpus)]
    print(devices)
    ...
    with strategy.scope():
        ...
        ds_train = strategy.distribute_datasets_from_function(train.distribute_dataset_fn(train_batch_size))
        ds_eval = strategy.distribute_datasets_from_function(eval.distribute_dataset_fn(eval_batch_size))
